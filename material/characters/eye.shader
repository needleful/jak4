...
uniform float iris_scale: hint_range(0.01, 1) = 0.5;
uniform vec2 iris_offset = vec2(0.5);
uniform float look_distance:hint_range(-1.0, 1.0) = 0.0;
...
varying float iris_flip;

void vertex() {
	iris_flip = sign(VERTEX.x);
}
